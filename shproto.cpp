...
void shproto_packet_add_data(shproto_struct * shproto_packet, unsigned char tx_byte) {
    if (shproto_packet->len >= shproto_packet->buffer_size) return;
...
// receive bytes one by one and places them into struct. set 'ready' flag is we got valid packet.
void shproto_byte_received(shproto_struct * shproto_packet, unsigned char rx_byte) {
    ...
    case SHPROTO_FINISH:
        ...
        if (!shproto_packet->crc) shproto_packet->ready = 1;  // crc16 with data and crc itself gives zero result, if not, CRC error here
        break;
    default:
        ...
        if (shproto_packet->len < shproto_packet->buffer_size) shproto_packet->len++;
